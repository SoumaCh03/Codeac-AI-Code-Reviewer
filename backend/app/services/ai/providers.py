...
class BaseLLMProvider(ABC):
    ...
    def review_security(self, diff: str) -> List[AIFinding]:
        messages = [
            ("system", "You are an expert Security Engineer. Review the following Git Diff and find ONLY security vulnerabilities (OWASP Top 10, SQLi, XSS, CSRF, SSRF, RCE, Secrets). If no critical security issues are found, return an empty list."),
            ("user", "Diff to review:\n{diff}")
        ]
    ...
    def review_performance(self, diff: str) -> List[AIFinding]:
        messages = [
            ("system", "You are a Staff Backend Engineer. Review the following Git Diff and find ONLY severe performance bottlenecks (N+1 queries, memory leaks, blocking I/O, O(N^2) loops). Provide optimized alternatives. If none are found, return an empty list."),
            ("user", "Diff to review:\n{diff}")
        ]
        return self._execute_review(messages, diff)
    ...
    def review_architecture(self, diff: str) -> List[AIFinding]:
        messages = [
            ("system", "You are a Principal Architect. Review the Git Diff and point out ONLY major architectural smells (tight coupling, breaking SOLID, god objects). Provide clear refactoring suggestions. Ignore minor style issues."),
            ("user", "Diff to review:\n{diff}")
        ]
        return self._execute_review(messages, diff)
    ...
    def review_maintainability(self, diff: str) -> List[AIFinding]:
        messages = [
            ("system", "You are a Senior Developer. Review the Git Diff for maintainability issues (long functions, poor naming, missing tests, high cyclomatic complexity). If code is clean, return an empty list."),
            ("user", "Diff to review:\n{diff}")
        ]
        return self._execute_review(messages, diff)
